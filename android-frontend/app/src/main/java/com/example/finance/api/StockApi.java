...
public class StockApi {
    ...
    static class ThreadStock extends Thread{
        ...
        public void run(){
            resStr = GetURLContent.RequestWithBody("http","10.0.2.2","9999","/sys/"+mapping,parameters,values);
            //System.out.println("resStr:"+resStr);
            a = 1;
        }
    }
...
/*    public static R<Object> GetLatest(String tsCode) throws IOException, InterruptedException {
    ...
    public static R<Object> GetAlikeAll(String input,Integer page,Integer pageSize) throws IOException, InterruptedException {
        ...
        values.add(page);
        values.add(pageSize);
        ThreadStock ts=new ThreadStock(parameters,values,"stockInfo/show","POST");
        ts.start();
        while(ts.a==null){
            Thread.sleep(100);
        }
