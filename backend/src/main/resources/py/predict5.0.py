...
if __name__ == '__main__':
    ...
    df = pro.daily(ts_code='601669.SH', start_date='20170101', end_date='20230407')
    ...
    result = pd.DataFrame(torch.vstack((X, Y)).detach().numpy()).iloc[-7:]  # 将预测的7天未来数据(除去节假日)接在前面100天之后
    result.insert(4,'code','601699.SH',allow_duplicates=False)
    result.index =  [pd.Timestamp('2023-04-10 00:00:00'),
                                                pd.Timestamp('2023-04-11 00:00:00'),
                                                pd.Timestamp('2023-04-12 00:00:00'),
                                                pd.Timestamp('2023-04-13 00:00:00'),
                                                pd.Timestamp('2023-04-14 00:00:00'),
                                                pd.Timestamp('2023-04-17 00:00:00'),
                                                pd.Timestamp('2023-04-18 00:00:00')]
    result.index.name = 'date'
    result.columns = ['open', 'close', 'high', 'low','code']
    Rarray = np.array(result)
    Rlist = Rarray.tolist()
    print(result)
